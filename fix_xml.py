# Repair the missing and incorrect elements in the XML file of the dataset.
import os
import sys
...
def fix_xml(filename):
    ...
    size = root.find("size")
    if size==None: #Check if size exists
        img_width,img_height,img_depth = find_image_size(filename) 
        #If it doesn't exist, find the image size
    ...
    else: #If it exists, check whether width and height are 0
...
for image in img_list: #Traverse file
    fields = image.split(".")
    filename_wo_extension = fields[0]
    extention = fields[1]

    if extention == 'xml': #Analyzing XML file to recover data
